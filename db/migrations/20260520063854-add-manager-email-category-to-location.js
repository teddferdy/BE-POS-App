...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    if (!table.managerName) {
      await queryInterface.addColumn('location', 'managerName', { type: Sequelize.STRING });
    }
    if (!table.email) {
      await queryInterface.addColumn('location', 'email', { type: Sequelize.STRING });
    }
    if (!table.category) {
      await queryInterface.addColumn('location', 'category', { type: Sequelize.STRING });
    }
  },
