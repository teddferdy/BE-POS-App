...
module.exports = {
  async up(queryInterface, Sequelize) {
    const [tableCheck] = await queryInterface.sequelize.query(
      `SELECT to_regclass('public.category_store') IS NOT NULL AS exists`
    )
    ...
    // 2. Migrate JSONB data → junction rows (safe to re-run, uses NOT EXISTS)
    const [colCheck] = await queryInterface.sequelize.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'category' AND column_name = 'store' AND table_schema = 'public'`
    )
    const storeColumnExists = colCheck.length > 0

    if (storeColumnExists) {
