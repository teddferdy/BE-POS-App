...
async function run() {
  ...
  for (const u of users) {
    console.log(
      `  [${u.id}] ${u.userName} / ${u.fullName} | role:${u.roleType} | store:${u.store} | pwd:${(u.password || '').slice(0, 20)}...`
    )
  }

  // Check categories
  const [cats] = await sequelize.query(
    `SELECT id, name, store FROM category WHERE "deletedAt" IS NULL AND status = 'active' LIMIT 20`
  )
  console.log('\n=== CATEGORIES ===')
