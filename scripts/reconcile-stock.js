...
const {
  getDiscrepancies,
  reconcile
} = require('../api/service/reconcileService')
...
async function main() {
  const mode = has('fix') ? arg('direction', 'store-to-global') : 'report'
  ...
  if (mode === 'report') {
    ...
    console.log(
      `Rekon (report) | store=${storeId || 'all'} | produk=${productId || 'all'}`
    )
  ...
  console.log(
    `Rekon (fix, ${result.direction}) selesai. Perubahan: ${result.total}`
  )
  for (const c of result.changes) {
    console.log(
      `  product#${c.productId}${c.store ? ` store=${c.store}` : ''} ${c.field}: ${c.before} -> ${c.after}`
    )
  }
  await db.sequelize.close()
  process.exit(0)
