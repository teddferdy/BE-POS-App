...
const ESC_POS = {
  ...
  // QR Code
  QR_MODEL_1: Buffer.from([0x1D, 0x28, 0x6B, 0x04, 0x00, 0x31, 0x41, 0x31, 0x00]),
  QR_MODEL_2: Buffer.from([0x1D, 0x28, 0x6B, 0x04, 0x00, 0x31, 0x41, 0x32, 0x00]),
  QR_SIZE: (size) => Buffer.from([0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x43, size]),
  QR_ERROR_CORRECTION: (level) => Buffer.from([0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x45, level]),
  QR_STORE_DATA: (data) => {
...
class ThermalPrinter {
  ...
  generateReceipt(data) {
    ...
    // Items header
    buffers.push(Buffer.from(this.formatLine('Item', 'Qty', 'Harga', 'Total') + '\n'));
    buffers.push(Buffer.from('-'.repeat(this.columns) + '\n'));
    
    // Items
    for (const item of items) {
      const name = this.truncate(item.name || item.nameProduct || 'Item', this.columns - 10);
      const qty = item.qty || item.quantity || 1;
      const price = this.formatNumber(item.price || item.sellingPrice || 0);
      const total = this.formatNumber((item.price || item.sellingPrice || 0) * qty);
      
      buffers.push(Buffer.from(`${name}\n`));
      buffers.push(Buffer.from(this.formatLine('', `${qty}x`, price, total) + '\n'));
  ...
  formatNumber(num) {
    return new Intl.NumberFormat('id-ID', { minimumFractionDigits: 0 }).format(Math.abs(num));
  }
  ...
  async connectSerial() {
    ...
    throw new Error('Serial connection not implemented - install serialport package');
  }

  async connectNetwork() {
    if (!this.ipAddress) throw new Error('IP address required for network printer');
  ...
  async connectBluetooth() {
    ...
    throw new Error('Bluetooth printing only supported on macOS via thermal-bt.py');
  }
  ...
  async print(data) {
    ...
    const receiptData = typeof data === 'object' ? this.generateReceipt(data) : data;
  ...
  async printBluetooth(data) {
    ...
    const mac = this.macAddress || process.env.BT_PRINTER_MAC || '86:67:7A:E4:30:C7';
